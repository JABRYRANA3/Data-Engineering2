"""
Generate synthetic match-picks data for DE2 Lab 3 - Track A (Esports).
Produces data/match_picks.csv with 200 matches x 5 heroes = 1000 picks.
Heroes have skewed popularity (some heroes appear in more matches than others)
to make the skew analysis interesting.
"""
...
# 50 heroes with skewed popularity weights (Zipf-like)
heroes = [f"hero_{i:02d}" for i in range(1, 51)]
weights = [1.0 / (i + 1) for i in range(len(heroes))]  # hero_01 most popular, hero_50 least
...
with open(out_file, "w", newline="", encoding="utf-8") as f:
    ...
    for match_id in range(1, n_matches + 1):
        # Sample 5 distinct heroes weighted by popularity
        chosen = set()
        attempts = 0
